...
<!-- Header Empresa -->
<div class="header">
    <table style="text-align: left; border:1px solid black; border-radius: 10px; width: 100%; font-size: 12px;">
        <tr>

            <td colspan="4" style="text-align: center;">

                <h2>{{$empresa->name}} | {{$datos->whereHouse->name}}</h2>
                <h3>COTIZACIÓN | <b>{{$datos->order_number}}</b> | {{date('d-m-Y H:s:i',strtotime($datos->created_at))}}
                </h3>
        ...
        <tr>
            <td>Estado: <br/> <b>{{$datos->sale_status??''}}</b></td>
            <td>Vendedor: <br/> {{$datos->seller->name??''}}  {{$datos->seller->last_name??''}} </td>

            <td colspan="1">Cliente: <br/> {{$datos->customer->name??'' ." ". $datos->customer->last_name??''}}
                // {{$datos->customer->address??''}}</td>
            {{--            <td colspan="2">Mecanico: {{$datos->mechanic->name??'S/N'}} {{$datos->mechanic->lastname??''}}</td>--}}
        </tr>

    </table>
    <!-- Tabla Productos -->
    <table class="tabla-productos{{ $datos->sale_status == 'Anulado' ? '-anulado' : '' }}" width="100%" border="1"
