...
function OrderCloseKardex($record, $isEntry = false, $operation = ''): bool
{
    ...
    foreach ($salesItem as $item) {
        ...
        // Verifica si el inventario existe y no es servicio
        if (!$inventory || $inventory->product->is_service) {
            continue;
        }

        try {
            // Si es entrada (anulación), devolver al inventario
            if ($isEntry) {
                app(KardexService::class)->registrarAnulacionVenta($sale, $item);
            } else {
                // Si es salida (cierre normal), registrar como venta
                app(KardexService::class)->registrarVenta($sale, $item);
            }
        } catch (\Exception $e) {
            Log::error("Error al crear Kardex para orden: {$item->id}", ['error' => $e->getMessage()]);
        }
    }
